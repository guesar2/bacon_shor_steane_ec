...
p_kinds = list(summary_dict["0_0_0"].keys())

p_upper_kinds = [i+"_upper" for i in p_kinds]
file_content = 'p ' + ' '.join(p_kinds) + ' ' + ' '.join(p_upper_kinds) + '\n'

probabilities = np.logspace(-6, -1, num=100)
...
for p in probabilities:
    ...
    file_content += "%.15f" % p + " " + " ".join("%.15f" % p_dict[p_kind] for p_kind in p_kinds) + " " + " ".join("%.15f" % p_dict_upper[p_kind] for p_kind in p_upper_kinds) + "\n"


data_filename = os.path.join(os.path.dirname(input_dir), 'results.dat') 
