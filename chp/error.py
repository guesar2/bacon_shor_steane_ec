...
class ErrorRatesAlternative(object):
	...
	def __init__(self, error_dict, error_kind=1):
		if 'error_kind' in list(error_dict.keys()):
...
def read_error(filename):
	...
	try:
		...
		one_qubit = [(str(x),y) for x,y in list(es['one_qubit_ratio'].items())]
		two_qubit = [(str(x),y) for x,y in list(es['two_qubit_ratio'].items())]
...
def pick_error(error_ratio):
	...
	"""
	key,value = list(zip(*error_ratio))
	...
	if sampling == 'old':
		for g in circ.gates[::-1]:
			...
			if r<rate:
				...
				error_ratio = [(str(x),y) for x,y in list(error_ratio_dic.items())]
				kind = error_info.error_kind
				if len(g.qubits) == 1:
					if ((kind==2 or kind==3) and g.qubits[0].qubit_type=='ancilla'):
						continue
