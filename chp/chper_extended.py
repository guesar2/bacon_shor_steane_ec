...
class Chper(object):
	def __init__(self, circ, num_d_qub, num_a_qub, stabs, destabs,
		...
		self.replacements = {'C':[('C', [0,1])],
			...
			'CNOT':[('C', [0,1])],
			'CZ':[('H', [1]), ('C', [0,1]), ('H', [1])],
			'CY':[('P', [1]), ('P', [1]), ('P',[1]),('C',[0,1]),('P',[1])],
            		'H':[('H', [0])],
			'P':[('P', [0])],
			'X':[('H', [0]),('P', [0]),('P', [0]),('H', [0])],
			'Y':[('P', [0]),('P',[0]),('P', [0]),('H', [0]),('P', [0]),('P', [0]),('H', [0]),('P',[0])],
			'Z':[('P', [0]),('P', [0])],
			'Measure':[('M', [0])],
			'MeasureZ':[('M', [0])],
            		'MeasureZDestroy':[('M', [0])],
		...
		if self.input_output_files:
			path = "/var/tmp/"+str(rd.randint(0, 10**9))+str(rd.randint(0, 10**9))+str(rd.randint(0, 10**9))+"/"
			#print(path)
			self.directory = os.path.dirname(path)
			if not os.path.exists(self.directory):
				os.makedirs(self.directory)
			self.input_file, self.n_gates = self.make_input_file(self.directory+"/messtempinput")
			self.arrange_stabilizers()
			self.input_stab_file = self.make_input_stab_file(self.directory+"/messtempstatet")
		else:
	...
	def parse_output(self, filename_or_string, output_file=True):
		...
		destabs = destabs_stabs[:int((len(destabs_stabs)-1)/2)]
		stabs = destabs_stabs[int((len(destabs_stabs)+1)/2):]
	...
	def run(self, chp_location='./chp_extended', output_filename='/tmp/messtempoutput',
		...
		if self.input_output_files:
			output_filename = self.directory+"/messtempoutput"
			command = ('%s -q %s %s %s %s %s %s > %s'%(chp_location, self.input_file, 
