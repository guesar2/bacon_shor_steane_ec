...
def read_error_info(error_info_or_file):
    ...
    if 'error_dict' in locals():
        if 'error_rates' in error_dict:
            ...
            one_qubit = [(str(x),y) for x,y in list(error_dict['one_qubit_ratio'].items())]
            two_qubit = [(str(x),y) for x,y in list(error_dict['two_qubit_ratio'].items())]
...
def pre_run_MC(error_subsets, operation, error_info, n_proc, output_folder, n_runs_total,
    ...
    if sampling == 'old':
        ...
        faulty_gates = list(error_info.dic.keys())
    ...
    if decoder[:3] == 'max':   
        ...
        try:
            if gamma != None and init_vec != None:
                ...
                if len(list(synd2corr_dict.keys())[0]) != int(log(len(tgvec),2)) or len(list(synd2corr_dict.keys())[0]) != int(log(len(tistate),2)):
                    errorflag = 1
