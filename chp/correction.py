...
class Surface_Code_ion_trap:
    ...
    def generate_logical_state_surf17(cls, stabilizer_list, ancillae_indexes,
        ...
        stabs_circ.to_ancilla(list(range(9,18)))       
...
class Flag_Correct:
    ...
    def generate_Reichardt_d3_1_flag(cls, meas_errors, Is_after_two, stabs='Z',
        ...
        Reich_circ.to_ancilla(list(range(n_data, n_data+n_anc)))
    ...
    def generate_one_flagged_stab(cls, i_first_anc, stabilizer, flags, 
        ...
        if to_ancilla:
            stab_circ.to_ancilla(list(range(i_first_anc, i_first_anc+n_flags+1)))
    ...
    def QEC_split_qubits(cls, Pauli_type='X', log_qubit=0, last_round=True):
        ...
        for i_stab in range(len(stabs)):
            ...
            stab_circ.to_ancilla(list(range(n_total, n_total+2)))
...
class Bare_Correct:
    ...
    def generate_bare_meas(cls, n_data, stabilizer_list, input_I_round=False, 
        ...
        if parallel:
            ...
            bare_meas_circ.to_ancilla(list(range(n_data, n_data+n_ancilla)))
...
class Cat_Correct:
    ...
    def create_stabs_d5_color_code(cls, initial_I, stabs, meas_errors, Is_after_two,
        ...
        octagon_range = stabs_indices[0][:] + list(range(n_q,n_q+8))
    ...
    def create_4_cat_verify(cls,stabilizer=['X','X','X','X']):
        ...
        final_circ = Circuit()
        final_circ.add_gate_at(list(range(4,9)),final_gate)
        ...
        final_circ.to_ancilla(list(range(4,9)))
    ...
    def create_4_cat_no_verify(cls,stabilizer):
        ...
        cat_circuit.to_ancilla(list(range(4,8)))
    ...
    def create_8_cat_verify(cls, meas_errors=True,
        ...
        cat_circuit.replace_qubit_ids(list(range(8,18)))
...
class Steane_Correct:
    ...
    def steane_syndrome(cls,ecc, redundancy =1, FT=True, kind='GFI'):
        ...
        synd_z_circ.to_ancilla(list(range(n_data, 2*n_data)))
        ...
        synd_x_circ.to_ancilla(list(range(n_data,2*n_data)))
    ...
    def FT_encoded_zero_Steane(cls, set_ancilla=True):
        ...
        circ1.join_circuit_at(list(range(7,14)), circ2)
        ...
        if set_ancilla:  circ1.to_ancilla(list(range(7,14)))
    ...
    def detect_errors(cls, error_to_correct, n_data, initial_trans=False):
        ...
        circ.to_ancilla(list(range(n_data, 2*n_data)))
    ...
    def steane_syndrome(cls, ancilla_parallel=False):
        ...
        main_circ.to_ancilla(list(range(n_data, n_initial + 2*n_ancilla)))
    ...
    def steane_syndrome_old(cls, ecc, redundancy=1):
        ...
        for i_redund in range(redundancy):
            ...
            synd_circ.replace_qubit_ids(list(range(n_data, n_data+2*n_anc)))
            
            err_det_circ, m_gates_synd = cls.detect_errors('Z', n_data)
            synd_circ.join_circuit_at(list(range(n_data+n_anc)), err_det_circ)
            synd_circ.to_ancilla(list(range(n_data,n_data+2*n_anc)))
            main_circ.join_circuit_at(list(range(n_data)), synd_circ)
...
class Knill_Correct:
    ...
    def detect_errors(cls, n_data, order='normal', initial_trans=False):
        ...
        circ.to_ancilla(list(range(n_data, 3*n_data)))
    ...
    def knill_syndrome(cls, ecc):
        ...
        cnot_qubits = list(range(n_data, n_data+n_ancilla)) + \
                    list(range(n_data+2*n_ancilla, n_data+3*n_ancilla))
        ...
        meas_Z_qubits = list(range(n_data, n_data + n_ancilla)) 
        bell_pair_circ.join_circuit_at(meas_Z_qubits, meas_Z_circ)

        bell_pair_circ.to_ancilla(list(range(n_data, n_data + 4*n_ancilla)))
